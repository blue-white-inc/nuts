...
function Nuts(opts) {
	...
	// Downloads the latest for specific channel for platform (autodetect platform if not specified)
	this.router.get('/download/channel/:channel', this.onDownload);
	this.router.get('/download/channel/:channel/:platform', this.onDownload);

	// Download specific version on specific channel for platform (autodetect platform if not specified)
	this.router.get('/download/channel/:channel/version/:tag', this.onDownload);
	this.router.get('/download/channel/:channel/version/:tag/file/:filename', this.onDownload);
	this.router.get('/download/channel/:channel/:platform/version/:tag', this.onDownload);
	this.router.get('/download/channel/:channel/:platform/version/:tag/file/:filename', this.onDownload);

	// Download specific version for platform (autodetect platform if not specified)
	this.router.get('/download/version/:tag', this.onDownload);
	this.router.get('/download/version/:tag/:platform', this.onDownload);

	// Download specific filename for tag/version
	this.router.get('/download/:tag/:filename', this.onDownload);
	this.router.get('/download/:tag/:channel/:filename', this.onDownload);

	// Download latest version for platform
	this.router.get('/download', this.onDownload);
	this.router.get('/download/:platform', this.onDownload);
	this.router.get('/feed/channel/:channel.atom', this.onServeVersionsFeed);

	if (this.opts.project === 'blue-central') {
		// These routes are only for BLUE-Central auto update
		this.router.get('/update', this.onUpdateRedirect);
		this.router.get('/update/:platform/:version', this.onUpdate); // Specific version for platform
		this.router.get('/update/channel/:channel/:platform/:version', this.onUpdate); // Specific version of specific channel on platform
	...
	this.router.get('/notes', this.onServeNotes);
...
Nuts.prototype.performPromise = function(name, arg, fn) {
	var that = this;
	fn = fn || function() { };

	return new Promise((resolve, reject) => {
		this.perform(name, arg, function (next) {
			Promise.resolve()
				.then(function() {
...
Nuts.prototype.serveAsset = function(req, res, version, asset) {
	...
	return that.init()
	.then(function() {
		return that.performPromise('download', {
			req: req,
			version: version,
			platform: asset
		}, function() {
...
Nuts.prototype.onDownload = function(req, res, next) {
	...
	if (!filename) {
		...
		if (!platform) {
			if (that.opts.bpkFormat) {
				platform = platforms.BLUE_CENTRAL;
			} else {
	...
	.then(function(version) {
		...
		if (!asset) {
			throw new Error("No download available for platform "+platform+" for version "+version.tag+" ("+(channel || "beta")+")");
		}
...
Nuts.prototype.getFullUrl = function(req) {
	if (this.opts.useSecureTransferProtocol === true) {
...
Nuts.prototype.onUpdate = function(req, res, next) {
	...
	.then(function(versions) {
		...
		// Properly handle latest tag for channels other than stable.  This was 
		// causing electron to always update even if you have the latest version.
		var latestTag = (channel !== 'stable') ? `${latest.tag}-${channel}` : latest.tag;
		if (!latest || latestTag === tag) {
