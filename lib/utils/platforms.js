...
function detectPlatform(platform) {
    ...
    // This is a Blue Central Firmware Package
    if(_.contains(name, 'blue_central') || hasSuffix(name, '.bpk')) {
        prefix = platforms.BLUE_CENTRAL;
    }
    ...
    if (!suffix) {
        if (prefix == platforms.OSX) {
            suffix = '64';
        } else if (prefix !== platforms.BLUE_CENTRAL) {
            suffix = '32';
        }
    }
...
function resolveForVersion(version, platformID, opts) {
    opts = _.defaults(opts || {}, {
        // Order for filetype
        filePreference: ['.exe', '.dmg', '.deb', '.rpm', '.tgz', '.tar.gz', '.zip', '.nupkg', '.bpk'],
        wanted: null
    });
